  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
        if (mesasSolicitadas <= mesasDisponibles) {
            resolve("Mesas disponibles, BIENVENIDO!");
        } else {
...
function enviarConfirmacionReserva(nombreCliente) {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
        const exito = Math.random() > 0.2;  // Simula un 80% de éxito en el envío del correo
        if (exito) {
...
async function hacerReserva(nombreCliente, mesasSolicitadas) {
  try {
    ...
    console.log(disponibilidad);  // Si hay mesas disponibles, muestra el mensaje de bienvenida

    if(disponibilidad=== "Mesas disponibles, BIENVENIDO!") {
      const confirmacion = await enviarConfirmacionReserva(nombreCliente);  
      console.log(confirmacion);  // Si el correo se envió correctamente, muestra el mensaje de confirmación
    }else{
        console.log("No se puede realizar reserva, no hay mesas disponibles");
    }
